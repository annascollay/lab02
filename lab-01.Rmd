...
**because some of the values aren't numerical data and so can't be averaged**
...
visitors_with_nations %>% 
  mutate(is_free = ifelse(admission == "Free", 1, 0)) %>%
  group_by(nation) %>%
  summarize(mean(is_free))
...
visitors_with_nations%>%
  ...
  filter(nation == "Scotland") %>%
    summarise(
      ...
      mean(n_2022, na.rm = TRUE)) %>%
mean(n_2021, na.rm = TRUE) = "mean2021"
   mutate(167338.3 - 80861.17) %>%
  mutate(86477.13 / 167338.3 * 100)
